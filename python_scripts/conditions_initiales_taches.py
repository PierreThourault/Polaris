...
def conditions_initiales_P0_FOC(dataset_params, deck_gen_params, facility_spec, iconfig):
    ...
    random_mode = 0                      # 0: aléatoire cumulatif progressif
    ...
    if bool_pert_P0:
        ...
        if random_mode == 0:
            for k in range(num_perturbations - 1):
                ...
                deck_gen_params["p0"][iconfig, beam_idx, :, k+1:] -= power_loss_quanta
            ...
            for k, beam_idx in enumerate(selected_beams):
                deck_gen_params["p0"][iconfig, beam_idx, :, k+1:] -= (cfg["n_quanta"] * power_loss_quanta)

        elif random_mode == 2:
            for beam, n_quanta in zip(cfg["affected_beams"], cfg["num_quanta"]):
                deck_gen_params["p0"][iconfig, beam, :, :] -= n_quanta * power_loss_quanta
